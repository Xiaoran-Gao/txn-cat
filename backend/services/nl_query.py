...
数据库schema如下：
- transactions(id, date DATE, raw_description TEXT, display_description TEXT, display_description_source TEXT, raw_product_info TEXT, display_product_info TEXT, amount REAL, currency TEXT, category_id INTEGER, subcategory_id INTEGER, classification_confidence INTEGER, classification_review_status TEXT, classification_review_reason TEXT, source TEXT, is_categorized INTEGER, created_at TIMESTAMP)
...
- raw_description 是原始账单描述，display_description 是清洗后用于展示和搜索的交易描述。raw_product_info/display_product_info 是商品或服务明细，判断用途时应同时参考，优先使用 display_product_info。
